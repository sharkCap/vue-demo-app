...
const changelogData = ref([
  ...
  "changes": {
    ...
    "bugfixes": [
      ...
      "安全/dependency audit: force `basic-ftp` 到 `5.2.1` 用于 这个 CRLF command-injection 修复 和 bump Hono plus `@hono/node-server` in production resolution paths.",
      ...
      "Windows/更新: 添加 heap headroom 到 Windows `pnpm b界面ld` steps during dev 更新s 以便 更新 preflight b界面lds stop failing on low default Node memory.",
      "插件 SDK: export 这个 channel 插件 base 和 web-search config contract through 这个 public package 以便 插件s 可以 use them without private imports.",
  ...
  "changes": {
    ...
    "bugfixes": [
      ...
      "Agents/history 和 replies: buffer phaseless OpenAI WS text until 一个 real assistant phase arrives, keep replay 和 SSE history sequence tracking aligned, hide commentary 和 leaked tool XML 从 user-visible history, 和 keep history-based follow-up replies on `final_answer` text only. (#61729, #61747, #61829, #61855, #61954)  和 contributors.",
      "Control 界面: show `/tts` audio replies in webchat, detect mistaken `?token=` auth links 具有 这个 correct `#token=` hint, 和 keep Copy, Canvas, 和 mobile exec-approval 界面 从 covering chat content on narrow screens. (#54842, #61514, #61598)",
      ...
      "Providers/Anthropic: preserve thinking blocks 用于 Claude Opus 4.5+, Sonnet 4.5+, 和 newer Claude 4-family models 以便 prompt-cache pre修复es keep matching, 和 skip `service_tier` injection on OAuth-authenticated stream wrapper requests 以便 Claude OAuth streaming stops failing 具有 HTTP 401. (#60356, #61793)",
      ...
      "Gateway/status 和 containers: auto-bind 到 `0.0.0.0` inside Docker 和 Podman environments, 和 probe local TLS gateways over `wss://` 具有 self-signed fingerprint forwarding 以便 container startup 和 loopback TLS status checks work again. (#61818, #61935)  和 contributors.",
      ...
      "插件s/channels: keep bundled channel artifact 和 secret-contract loading stable under lazy loading, preserve 插件-schema defaults during install, 和 修复 Windows `file://` plus native-Jiti 插件 loader paths 以便 onboarding, doctor, `openclaw secret`, 和 bundled 插件 installs work again. (#61832, #61836, #61853, #61856)  和 contributors.",
      ...
      "插件s/provider hooks: stop recursive provider snapshot loads 从 overflowing 这个 stack during 插件 initialization, while still preserving cached nested provider-hook results. (#61922, #61938, #61946, #61951)",
      ...
      "Tools/web_fetch 和 web_search: 修复 `TypeError: fetch failed` caused 通过 undici 8.0 enabling HTTP/2 通过 default; pinned SSRF-guard dispatchers now explicitly set `allowH2: false` 到 restore HTTP/1.1 behavior 和 keep 这个 custom DNS-pinning lookup compatible. (#61738, #61777)",
      ...
      "Cron: load `jobId` into `id` when 这个 on-disk store omits `id`, matching doctor migration 和 修复ing `unknown cron job id` 用于 hand-edited `jobs.json`.",
  ...
  "changes": {
    "features": [
      ...
      "Providers: 添加 bundled Qwen, Fireworks AI, 和 StepFun providers, plus MiniMax TTS, Ollama Web Search, 和 MiniMax Search 集成s 用于 chat, speech, 和 search workflows. (#60032, #55921, #59318, #54648)",
      ...
      "插件s: 添加 插件-config T界面 prompts 到 g界面ded onboarding/setup flows, 和 添加 `openclaw 插件s install --force` 以便 existing 插件 和 hook-pack targets 可以 be replaced without using 这个 dangerous-code override flag. (#60590, #60544)",
    ...
    "bugfixes": [
      "安全: preserve restrictive 插件-only tool allowlists, req界面re owner access 用于 `/allowlist 添加` 和 `/allowlist 移除`, fail closed when `before_tool_call` hooks crash, block browser SSRF redirect bypasses earlier, 和 keep non-interactive auth-choice inference scoped 到 bundled 和 already-trusted 插件s. (#58476, #59836, #59822, #58771, #59120)  和 @pgondhi987.",
      ...
      "Providers/OpenAI and reply delivery: preserve native `reasoning.effort: \"none\"` 和 strict schemas where 支持ed, 添加 GPT-5.4 assistant `phase` metadata across replay 和 这个 Gateway `/v1/responses` layer, 和 keep commentary buffered until `final_answer` 以便 web chat, session previews, embedded replies, 和 Telegram partials stop leaking planning text. 修复es #59150, #59643, #61282.",
      "Telegram: 修复 current-model checks in 这个 model picker, HTML-format non-default `/model` confirmations, explicit topic replies, persisted reaction ownership across restarts, caption-media placeholder 和 `file_id` preservation on download failure, 和 upgraded-install inbound image reads. (#60384, #60042, #59634, #59207, #59948, #59971) , @GitZhangChi, @dashhuang, @samzong, @v1p0r, 和 @neeravmakwana.",
      ...
      "Discord: keep REST, webhook, 和 monitor traffic on 这个 configured proxy, preserve component-only media sends, honor `@everyone` 和 `@here` mention gates, keep ACK reactions on 这个 active account, 和 split voice connect/playback timeouts 以便 auto-join is more reliable. (#57465, #60361, #60345)",
      ...
      "Gateway/startup: default `gateway.mode` 到 `local` when unset, detect PID recycling in gateway lock files on Windows 和 macOS, 和 show startup progress 以便 healthy restarts stop getting blocked 通过 stale locks. (#54801, #60085, #59843)  和 @TonyDerek-dot.",
      ...
      "CLI/skills JSON: route `skills list --json`, `skills info --json`, 和 `skills check --json` output 到 stdout instead of stderr 以便 machine-readable consumers receive JSON on 这个 expected stream again. (#60914; 修复es #57599; landed 从 contributor PR #57611 通过 @Aftabbs)",
      ...
      "Exec approvals: reuse durable exact-command `allow-always` approvals in allowlist mode 以便 identical reruns stop prompting, 和 tighten Windows interpreter/path approval handling 以便 wrapper 和 malformed-path cases fail closed more consistently. (#59880, #59780, #58040, #59182) , @SnowSky1, 和 @pgondhi987.",
      ...
      "插件s/Kimi Coding: parse tagged tool calls 和 keep Anthropic-native tool payloads 以便 Kimi coding endpoints execute tools instead of echoing raw markup. (#60051, #60391)  和 @Eric-Guo.",
      ...
      "Tools/web_search (Kimi): when `tools.web.search.kimi.baseUrl` is unset, inherit native Moonshot chat `baseUrl` (`.ai` / `.cn`) 以便 China console keys authenticate on 这个 same host as chat. 修复es #44851.",
  ...
  "changes": {
    "features": [
      ...
      "插件s/hooks: 添加 `before_agent_reply` 以便 插件s 可以 short-circ界面t 这个 LLM 具有 synthetic replies after inline actions.",
      ...
      "Diffs: 添加 插件-owned `viewerBaseUrl` 以便 viewer links 可以 use 一个 stable proxy/public origin without passing `baseUrl` on every tool call.  Related #59227.",
    ...
    "improvements": [
      "插件s/xAI: move `x_search` settings 从 这个 legacy core `tools.web.x_search.*` path 到 这个 插件-owned `插件s.entries.xai.config.xSearch.*` path, standardize `x_search` auth on `插件s.entries.xai.config.webSearch.APIKey` / `XAI_API_KEY`, 和 migrate legacy config 具有 `openclaw doctor --修复`.",
    ...
    "bugfixes": [
      ...
      "Agents/subagents: pin admin-only subagent gateway calls to `operator.admin` while keeping `agent` at least privilege, so `sessions_spawn` no longer dies on loopback scope-upgrade pairing with `close(1008) \"pairing req界面red\"`.",
  ...
  "changes": {
    "features": [
      ...
      "ZAI/models: 添加 `glm-5.1` 和 `glm-5v-turbo` 到 这个 bundled Z.AI provider catalog.",
      "Agents/compaction: resolve `agents.defaults.compaction.model` consistently for manual `/compact` and other context-engine compaction paths, so engine-owned compaction uses the configured override model across runtime entrypoints. (#56710) Thanks @oliviareid-svg"
    ],
    "improvements": [],
    "bugfixes": [
      ...
      "LINE/runtime: resolve 这个 packaged runtime contract 从 这个 b界面lt `dist/插件s/runtime` layout 以便 LINE channels start correctly again after global npm installs on `2026.3.31`.",
  ...
  "changes": {
    ...
    "bugfixes": [
      ...
      "Docker/setup: force B界面ldKit 用于 local image b界面lds (including sandbox image b界面lds) 以便 `./docker-setup.sh` no longer fails on `RUN --mount=...` when hosts default 到 Docker's legacy b界面lder.  界面-china.",
  ...
  "changes": {
    ...
    "bugfixes": [
      ...
      "Memory/QMD: keep `qmd embed` active in `search` mode too, 以便 BM25-first setups still b界面ld 一个 complete index 用于 later vector 和 hybrid retrieval.   和 @vincentkoc.",
      ...
      "OpenAI/Responses: omit disabled reasoning payloads across OpenAI, Codex, and Azure OpenAI request b界面lders so GPT-5 endpoints no longer reject `reasoning.effort: \"none\"`.",
      ...
      "界面/DOM safety: b界面ld 这个 chat delete-confirm popover 和 Canvas Host fallback status line 具有 DOM nodes instead of injected HTML strings. (#58269, #58266)",
      ...
      "Exec approvals: stop shell init-file flags 从 matching persisted script-path approvals, detect command-carrier inline eval in tools like `awk`, `find`, `xargs`, `make`, 和 `sed`, 和 treat 这个 awk family as interpreter-like 以便 allow-always decisions no longer persist interpreter paths. (#58369, #57842, #57772)",
      ...
      "Gateway/HTTP trust boundaries: ignore self-declared bearer scopes, deny dangerous HTTP tool-invoke paths 通过 default, req界面re write scope on `/v1/embeddings`, 和 bind OpenAI-compatible `/v1/chat/completions` plus `/v1/responses` ingress as non-owner 以便 external HTTP callers cannot self-upgrade access. (#57783, #57771, #57721, #57769, #57778)",
